...
ParseRelativeDate <- function(
  ...
  if (!is.na(snap_match[1, 1])) {
    ...
    return(switch(
      snap_unit,
      "CM" = if (type == "start") {
        lubridate::floor_date(base_date, "month")
      } else {
        lubridate::ceiling_date(base_date, "month") - lubridate::days(1)
      },
      "CQ" = if (type == "start") {
        lubridate::floor_date(base_date, "quarter")
      } else {
        lubridate::ceiling_date(base_date, "quarter") - lubridate::days(1)
      },
      "CY" = if (type == "start") {
        lubridate::floor_date(base_date, "year")
      } else {
        lubridate::ceiling_date(base_date, "year") - lubridate::days(1)
      }
    ))
  }
